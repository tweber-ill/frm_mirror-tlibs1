...
bool FontMap::LoadFont(FT_Face ftFace)
{
	...
	m_iPadH = nextpow2(m_iTileH)-m_iTileH;
	m_iPadW = nextpow2(m_iTileW)-m_iTileW;
	...
	m_iLargeW = nextpow2(m_iLargeW);
	m_iLargeH = nextpow2(m_iLargeH);
...
/*
// gcc -I/usr/include/freetype2 -o tst_font helper/gl.cpp helper/log.cpp -lm -lstdc++ -lfreetype -lGL -std=c++11
#include <fstream>

int main()
{
	FontMap map(DEF_FONT, 12);
	std::ofstream ofstr("font.map");
	map.dump(ofstr);

	map.dump(std::cout, map.GetOffset('T'));
	map.dump(std::cout, map.GetOffset('o'));
	map.dump(std::cout, map.GetOffset('b'));
	map.dump(std::cout, map.GetOffset('i'));
	return 0;
}
*/
